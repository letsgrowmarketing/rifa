  ...
  return (
    <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      <div className="mb-8">
        ...
        <p className="text-gray-600 mt-1">Encontre jogadores por nome, email, CPF ou número da rifa</p>
      </div>
      ...
      <div className="space-y-6">
        {results.length === 0 && searchTerm && !loading && (
          <div className="text-center py-12 bg-white rounded-lg shadow-sm border border-gray-200">
            ...
            <p className="text-gray-600">Tente ajustar os termos de busca ou o tipo de pesquisa.</p>
          </div>
        )}
              ...
              <div>
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h3 className="text-lg font-semibold text-gray-900">{result.user.nome}</h3>
                      <p className="text-gray-600">{result.user.email}</p>
                      <p className="text-sm text-gray-500">{result.user.cpf}</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-sm text-gray-500">Cadastrado em</p>
                    <p className="font-medium">{formatDate(result.user.data_cadastro)}</p>
                  </div>
                </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-4">
                  <div className="bg-blue-50 p-4 rounded-lg text-center">
                    ...
                    <p className="text-lg font-bold text-blue-900">{formatCurrency(result.userStats.totalDeposited)}</p>
                    <p className="text-sm text-blue-600">Total Depositado</p>
                  </div>
                  <div className="bg-green-50 p-4 rounded-lg text-center">
                    <DollarSign className="w-6 h-6 text-green-600 mx-auto mb-2" />
                    <p className="text-lg font-bold text-green-900">{formatCurrency(result.userStats.totalApproved)}</p>
                    <p className="text-sm text-green-600">Aprovado</p>
                  </div>
                  <div className="bg-purple-50 p-4 rounded-lg text-center">
                    <Hash className="w-6 h-6 text-purple-600 mx-auto mb-2" />
                    <p className="text-lg font-bold text-purple-900">{result.userStats.numbersCount}</p>
                    <p className="text-sm text-purple-600">Números</p>
                  </div>
                  <div className="bg-gray-50 p-4 rounded-lg text-center">
                    <Trophy className="w-6 h-6 text-gray-600 mx-auto mb-2" />
                    <p className="text-lg font-bold text-gray-900">{result.userStats.vouchersCount}</p>
                    <p className="text-sm text-gray-600">Comprovantes</p>
                  </div>
                </div>

                {result.numbers.length > 0 && (
                  <div>
                    <h4 className="font-medium text-gray-900 mb-2">Números da Rifa ({result.numbers.length})</h4>
                    <div className="grid grid-cols-6 sm:grid-cols-8 md:grid-cols-10 lg:grid-cols-12 gap-2">
                      {result.numbers.slice(0, 24).map((numero: NumeroRifa) => (
                        <div
                          key={numero.id}
                          className="bg-gray-100 text-center py-1 px-2 rounded text-sm font-medium"
                        >
                          {numero.numero_gerado}
                        </div>
                      ))}
                      {result.numbers.length > 24 && (
                        <div className="bg-gray-200 text-center py-1 px-2 rounded text-sm text-gray-600">
                          +{result.numbers.length - 24}
                        </div>
                      )}
                    </div>
              ...
              <div>
                <div className="flex items-start justify-between mb-4">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <h3 className="text-2xl font-bold text-gray-900">{result.numero.numero_gerado}</h3>
                      <p className="text-gray-600">Número da Rifa</p>
                    </div>
                  </div>
                  <div className="text-right">
                    <p className="text-sm text-gray-500">Sorteio</p>
                    <p className="font-medium">{result.sorteio?.nome || 'Sorteio não encontrado'}</p>
                  </div>
                </div>

                {result.user && (
                  <div className="bg-gray-50 rounded-lg p-4">
                    <h4 className="font-medium text-gray-900 mb-3">Proprietário do Número</h4>
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="font-medium text-gray-900">{result.user.nome}</p>
                        <p className="text-sm text-gray-600">{result.user.email}</p>
                        <p className="text-sm text-gray-500">{result.user.cpf}</p>
                      </div>
                      <div className="text-right">
                        <p className="text-lg font-bold text-green-600">{formatCurrency(result.userStats.totalApproved)}</p>
